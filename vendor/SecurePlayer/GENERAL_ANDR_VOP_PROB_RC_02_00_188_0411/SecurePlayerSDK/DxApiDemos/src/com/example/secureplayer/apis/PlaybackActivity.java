...
public class PlaybackActivity extends Activity implements VOCommonPlayerListener
{
	...
	VO_OSMP_PLAYER_ENGINE mPlayerType = DxConstants.getPlayerType();
	...
	private void initPlayer()
	{
		...
		VO_OSMP_RETURN_CODE nRet = mPlayer.init(mPlayerType, initParam);
		...
		if (mPlayer!= null && !DxConstants.isVideoSpecificsSet())
		{
			...
			if (subtitleCount > 0) {
				...
				for (int iIndex=0 ; iIndex<subtitleCount ; ++iIndex) {
					...
					if (propertyDescription != null) {
						subtitleName = propertyDescription;
					}
					else if (propertyLanguage != null && propertyCodec != null) {
						subtitleName = propertyLanguage + "//" + propertyCodec;
					}
					else if (propertyLanguage != null) {
						subtitleName = propertyLanguage;
					}
					else {
			...
			if (audioCount > 0) {
				...
				for (int iIndex=0 ; iIndex<audioCount ; ++iIndex) {
					...
					if (propertyDescription != null) {
						audioName = propertyDescription;
					}
					else if (propertyLanguage != null && propertyCodec != null) {
						audioName = propertyLanguage + "//" + propertyCodec;
					}
					else if (propertyLanguage != null) {
						audioName = propertyLanguage;
					}
					else {
						audioName = "Unknown";
					}
