...
function usage {
  ...
  echo ""
  echo "    update-second-round-testing-task | -srt <ticket-id>:     Update the second round testin subtask by posting the related links and commnets"
  echo ""
  echo "    publish-sdk-release | -pr <ticket-id>:                  Publish the current release" 
  echo ""
  echo "    email-after-release | -ear <ticket-id>:                 Send email to ??? to notify the relevant information about this release"
...
function clone_ticket {
  ...
  if ! [[ $release_date =~ ^[2-9]0[1-9]{2}-[0-1][0-9]-[0-3][0-9]$ ]]; then
    echo "Not a valid date format. Please retry with format YYYY-MM-DD."
    exit 1
  fi
  ...
  echo "New Ticket ${new_ticket_num} Created. Sent email to ${receiver}."
}
...
function file_release_doc_ticket {
  ...
  echo "${new_Android_docs_ticket} has been generated and post on doc subtask"
}
...
function cut_branch {
  ...
  echo "Published a new RC from master" 
}
...
function update_first_round_testing_task {
  ...
  echo "Updated first round testing ticket"
}
...
function update_second_round_testing_task {
  ...
  echo "Updated second round testing ticket"
}
...
function publish_sdk_release {
  ...
  echo "Adnroid-SDK/${release_date} published!"
}
...
function email_after_release {
  ...
  echo ${body} | mail -s "${subject}" ${receiver}
  echo "Release email has been sent to ${receiver}."
...
function fetch_release_info {
  if [ task != "-ct" ] && [ task != "clone-ticket" ]; then
...
function get_release_date {
  if [ "${release_date}" = "" ]; then
    ...
    echo ${release_date}
    if ! [[ $release_date =~ ^[2-9]0[1-9]{2}-[0-1][0-9]-[0-3][0-9]$ ]]; then
      echo "Not a valid date format. Please make sure the main release ticket name is in correct format: Android SDK Release Ticket YYYY-MM-DD"
      exit 1
    fi
    echo "Get release date = ${release_date}"
  fi
}
...
function can_move_on {
  ...
  echo "All previous steps have been finished and the curren step has not been started yet. OK to move on!"
}
...
function check_ticket_number_format {
  ...
  if ! [[ ${new_ticket_num} =~ ^[Pp][Bb][Aa]-[0-9]{1,}$ ]]; then
    echo "Not a valid PBA ticket format. Please retry with ticket format PBA-XXX."
    exit 1
  fi
  echo "Valid PBA ticket format."
}
...
echo "Task: " ${task}
echo "Ticket number: " ${new_ticket_num}
