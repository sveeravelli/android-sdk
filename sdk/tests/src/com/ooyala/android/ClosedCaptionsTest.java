...
public class ClosedCaptionsTest extends AndroidTestCase
{
  ...
  {
    ClosedCaptions cc = new ClosedCaptions(ContentItemTest.getTestJSON(TestConstants.TEST_DICTIONARY_CLOSED_CAPTIONS));
    assertEquals("http://ak.c.ooyala.com/1ndnAxMzpxA4MFMw8G-F7frGiDYD_15p/cc/148094784", cc.getURL().toString());
  ...
  public void testFetchAndGet()
  {
    ...
    assertEquals("two bytes meet. the first byte asks \"are you ill?\"\n\nthe second byte replies: \"no, just feeling a bit off.\"", caption.getText());
    caption = cc.getCaption("en", 19.900);
    assertEquals("alskdasdkja", caption.getText());
    caption = cc.getCaption("en", 40.300);
    assertEquals("The second string says: \"Please excuse my friend, he isnt null-terminated.\"", caption.getText());
    caption = cc.getCaption("fr", 0.889);
    assertEquals("FR: two bytes meet. the first byte asks \"are you ill?\"\n\nthe second byte replies: \"no, just feeling a bit off.\"", caption.getText());
    caption = cc.getCaption("fr", 19.900);
    assertEquals("FR: alskdasdkja", caption.getText());
    caption = cc.getCaption("fr", 40.300);
    assertEquals("FR: The second string says: \"Please excuse my friend, he isnt null-terminated.\"", caption.getText());
