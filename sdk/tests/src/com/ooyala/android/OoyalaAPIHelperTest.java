...
public class OoyalaAPIHelperTest extends AndroidTestCase {
  ...
  public void testObjectForSecureAPI() {
    ...
    try {
      ...
      assertTrue(json.getJSONObject("authorization_data").getJSONObject(TestConstants.TEST_VIDEO).getBoolean("authorized"));
    } catch (Exception e) {
      e.printStackTrace();
      fail();
    }
  ...
  public void testJSONForAPI() {
    Map<String,String> params = new HashMap<String,String>();
    params.put("device", "android");
    String json = OoyalaAPIHelper.jsonForAPI("http://player.ooyala.com", "/player_api/content_tree/embed_code/"+TestConstants.TEST_PCODE+"/"+TestConstants.TEST_VIDEO, params);
  ...
  public void testObjectForAPI() {
    Map<String,String> params = new HashMap<String,String>();
    params.put("device", "android");
    JSONObject json = OoyalaAPIHelper.objectForAPI("http://player.ooyala.com", "/player_api/content_tree/embed_code/"+TestConstants.TEST_PCODE+"/"+TestConstants.TEST_VIDEO, params);
    assertFalse(json.isNull("content_tree"));
    try {
      assertFalse(json.getJSONObject("content_tree").isNull(TestConstants.TEST_VIDEO));
      assertEquals(json.getJSONObject("content_tree").getJSONObject(TestConstants.TEST_VIDEO).getString("embed_code"), TestConstants.TEST_VIDEO);
    } catch (Exception e) {
      e.printStackTrace();
      fail();
    }
