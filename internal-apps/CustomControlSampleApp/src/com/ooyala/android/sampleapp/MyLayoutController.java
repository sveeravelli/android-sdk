...
/**
 * This LayoutController is a generic LayoutController that will work in most
 * cases (regardless of the containing Layout type). It uses basic controls and
 * allows additional overlays to be added. Fullscreening is done by opening a
 * full screen Dialog and filling it with a dynamically created
 * OoyalaPlayerLayout. Because of this, playback will be suspended and
 * subsequently resumed during this process. As a result, fullscreening is
 * slower than if the OoyalaPlayerLayout is embeded directly in the Activity's
 * base layout, that base layout is a FrameLayout, and the LayoutController used
 * is FastMyLayoutController.
 * 
 * @author jigish
 */
public class MyLayoutController extends AbstractOoyalaPlayerLayoutController {

  /**
   * Instantiate an MyLayoutController
   * 
   * @param l
   *          the layout to use
   * @param pcode
   ...
   */
  public MyLayoutController(OoyalaPlayerLayout l, String pcode,
      PlayerDomain domain) {
   ...
   */
  public MyLayoutController(OoyalaPlayerLayout l, String pcode,
      PlayerDomain domain, EmbedTokenGenerator generator) {
   ...
   *          the layout to use
   * @param p
   *          the instantiated player to use
   */
  public MyLayoutController(OoyalaPlayerLayout l, OoyalaPlayer p) {
   ...
   *          the embed domain to use
   * @param dcs
   *          the DefaultControlStyle to use (AUTO is default controls, NONE has
   *          no controls)
   */
  public MyLayoutController(OoyalaPlayerLayout l, String pcode,
      PlayerDomain domain, DefaultControlStyle dcs) {
   ...
   *          An embedTokenGenerator used to sign SAS requests
   * @param options
   *          Extra values, can be null in which case defaults values are used.
   */
  public MyLayoutController(OoyalaPlayerLayout l, String pcode,
      PlayerDomain domain, DefaultControlStyle dcs,
      EmbedTokenGenerator generator, Options options) {
  ...
  protected void doFullscreenChange(boolean fullscreen) {
    ...
    if (isFullscreen() && !fullscreen) { // Fullscreen -> Not Fullscreen
      ...
      setFullscreenControls(new MyFullscreenControls(_player, _fullscreenLayout));
